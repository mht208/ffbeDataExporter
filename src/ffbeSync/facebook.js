...
chrome.runtime.onMessage.addListener(function (msg, sender, data) {
    ...
    if (msg.type == 'get_facebook_variables') {
		...
		console.log("received get_facebook_variables request");
        chrome.runtime.sendMessage({type:"started", data:"facebookToken"});
		console.log("sending facebook_variables msg");
    console.log("fb_dtsg: " + document.body.innerHTML.match(/"name":"fb_dtsg","value":"([^"]+)"/)[1]);
    console.log("jazoest: " + document.body.innerHTML.match(/"name":"jazoest","value":"([^"]+)"/)[1]);
        chrome.runtime.sendMessage({
            type:"facebook_variables",
            data: {
                fb_dstg: document.body.innerHTML.match(/"name":"fb_dtsg","value":"([^"]+)"/)[1] /*document.getElementsByName("fb_dtsg")[0].value*/,
                jazoest: document.body.innerHTML.match(/"name":"jazoest","value":"([^"]+)"/)[1] /*document.getElementsByName("jazoest")[0].value*/
            }
        });


		console.log("sent facebook_variables msg");
